...
`;
        // ─── SVG Icons ─────────────────────────────────────────────────────────────
        function IconMeter() {
            return React.createElement('svg', { width: 15, height: 15, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' }, React.createElement('path', { d: 'M12 2v4' }), React.createElement('path', { d: 'm4.93 4.93 2.83 2.83' }), React.createElement('path', { d: 'M2 12h4' }), React.createElement('path', { d: 'm4.93 19.07 2.83-2.83' }), React.createElement('path', { d: 'M12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12Z' }), React.createElement('path', { d: 'm14 10-2 2' }));
        }
        ...
        function TimeseriesChart(props) {
            ...
            return React.createElement('div', { className: 'dup-card dup-chart-card' }, React.createElement('div', { className: 'dup-card-header' }, React.createElement('span', { className: 'dup-card-title' }, t('timeseriesTitle')), React.createElement('div', { className: 'dup-segmented' }, React.createElement('button', {
                type: 'button',
            ...
            }, t('toggleCalls')))), React.createElement('div', { className: 'dup-chart-container' }, hoveredPoint &&
                React.createElement('div', { className: 'dup-chart-tooltip' }, React.createElement('strong', null, hoveredPoint.row.date), ' : ', metric === 'tokens' ? formatTokens(hoveredPoint.val) : metric === 'cost' ? formatCostUsd(hoveredPoint.val) : formatNumber(hoveredPoint.val) + ' 次'), React.createElement('svg', { className: 'dup-chart-svg', viewBox: `0 0 ${width} ${height}`, preserveAspectRatio: 'none' }, React.createElement('defs', null, React.createElement('linearGradient', { id: 'dup-area-grad', x1: '0', y1: '0', x2: '0', y2: '1' }, React.createElement('stop', { offset: '0%', stopColor: '#007AFF', stopOpacity: '0.28' }), React.createElement('stop', { offset: '100%', stopColor: '#007AFF', stopOpacity: '0.01' }))), 
            // Baseline
        ...
        function UsageSidebarTab(props) {
            ...
            const loadAllData = React.useCallback((silent = false) => {
                ...
                Promise.all([
                    ...
                    callRpc('timeseries', { range, bucket: 'day' }),
            ...
            return React.createElement('div', { className: 'dup-root' }, 
            ...
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' } }, React.createElement('div', { className: 'dup-segmented' }, React.createElement('button', {
                ...
                disabled: refreshing,
                title: t('btnRefresh')
            }, React.createElement(IconRefresh, { spinning: refreshing }), refreshing ? t('btnRefreshing') : t('btnRefresh'))), 
            // Loading indicator
            loading &&
            ...
            summary &&
                ...
                // Total Cost
                React.createElement('div', { className: 'dup-card dup-kpi-card' }, React.createElement('span', { className: 'dup-kpi-label' }, t('kpiTotalCost')), React.createElement('span', { className: 'dup-kpi-val' }, formatCostCny(summary.cost.total)), React.createElement('span', { className: 'dup-kpi-sub' }, `${formatCostUsd(summary.cost.total)} (USD)`)), 
                // Cache Hit Rate
            ...
            React.createElement('div', { className: 'dup-card' }, React.createElement('div', { className: 'dup-card-header' }, React.createElement('span', { className: 'dup-card-title' }, t('breakdownTitle'))), modelBreakdown.length === 0
                ...
                : React.createElement('div', { className: 'dup-table-wrap' }, React.createElement('table', { className: 'dup-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', null, t('colModel')), React.createElement('th', null, t('colTokens')), React.createElement('th', null, t('colCost')), React.createElement('th', null, t('colCalls')), React.createElement('th', null, t('colCacheHit')))), React.createElement('tbody', null, modelBreakdown.map((row) => {
                    ...
                    return React.createElement('tr', { key: row.key }, React.createElement('td', { style: { fontWeight: 600 } }, React.createElement('div', null, row.key), React.createElement('div', { style: { marginTop: 3 } }, React.createElement(PriceModeBadge, { mode, t }))), React.createElement('td', null, formatTokens(row.tokens)), React.createElement('td', null, mode === 'unpriced'
            ...
            !details || details.rows.length === 0
                ? React.createElement('div', { className: 'dup-empty-desc', style: { padding: '16px 0', textAlign: 'center' } }, t('emptyList'))
                : React.createElement('div', { className: 'dup-table-wrap' }, React.createElement('table', { className: 'dup-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', null, t('colTimestamp')), React.createElement('th', null, t('colModel')), React.createElement('th', null, t('colOrigin')), React.createElement('th', null, t('colUsageDetail')), React.createElement('th', null, t('colCost')))), React.createElement('tbody', null, details.rows.map((r, idx) => React.createElement('tr', { key: r.ts + '-' + idx }, React.createElement('td', { style: { whiteSpace: 'nowrap' } }, formatTimestamp(r.ts)), React.createElement('td', null, React.createElement('div', { style: { fontWeight: 600 } }, r.model || '—'), React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-text-tertiary, #86909C)' } }, r.provider || '')), React.createElement('td', null, React.createElement('span', {
                    className: 'dup-badge',
                    style: {
        ...
        function InputDockPill(props) {
            const { t, ctx } = props;
            const [todaySummary, setTodaySummary] = React.useState(null);
            const [monthSummary, setMonthSummary] = React.useState(null);
            const [error, setError] = React.useState(null);
            const fetchMetrics = React.useCallback(() => {
                Promise.all([
                    callRpc('summary', { range: 'today' }).then((value) => { setTodaySummary(value); return value; }, (err) => { throw new Error(`today: ${String(err?.message ?? err)}`); }),
                    callRpc('summary', { range: 'month' }).then((value) => { setMonthSummary(value); return value; }, (err) => { throw new Error(`month: ${String(err?.message ?? err)}`); })
                ]).then(() => { setError(null); }, (err) => {
            ...
            }, React.createElement(IconMeter), React.createElement('span', null, t('dockPillText', { today: todayTokens, month: monthTokens })));
        }
        // ─── Sidebar Title Component ───────────────────────────────────────────────
        function UsageTabTitle(props) {
            return React.createElement('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 6 } }, React.createElement(IconMeter), React.createElement('span', null, props.t ? props.t('tabTitle') : '用量看板'));
        }
