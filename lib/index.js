...
async function volcengineCall(action, region, ak, sk, timeoutMs) {
    ...
    if (raw.byteLength > MAX_BODY_BYTES)
        throw new Error(`upstream body exceeds ${MAX_BODY_BYTES} bytes`);
    let body = null;
    try {
 ...
 */
function chatgptErrorText(json, fallback) {
    if (typeof json?.error_description === 'string' && json.error_description)
 ...
 */
async function refreshChatGPTToken(refreshToken, timeoutMs, proxyUrl) {
 ...
 */
let chatgptLoginError = null;
/** Step 1: request a one-time code from the device-authorization endpoint. */
async function chatgptDeviceStart(timeoutMs, proxyUrl) {
    ...
    void chatgptDevicePoll(deviceAuthId, userCode, intervalMs, proxyUrl).catch((e) => {
        chatgptLoginError = String(e?.message || e);
    });
 ...
 */
async function chatgptDevicePoll(deviceAuthId, userCode, initialIntervalMs, proxyUrl) {
...
async function chatgptExchangeCode(code, codeVerifier, timeoutMs = UPSTREAM_TIMEOUT_MS, proxyUrl) {
    const body = new URLSearchParams({
...
function chatgptLoginStatus() {
    if (!chatgptLogin || chatgptLogin.abort)
        return { active: false, error: chatgptLoginError };
    return {
        active: true,
 ...
 */
async function resolveChatGPTToken(timeoutMs, proxyUrl) {
    const now = Date.now();
 ...
 */
async function fetchChatGPTUsage(accessToken, accountId, timeoutMs, proxyUrl) {
    const headers = {
...
function parseChatGPTUsage(body) {
    ...
    const windowLane = (seconds) => {
        const n = Number(seconds);
        if (!Number.isFinite(n) || n <= 0)
            return null;
        const near = (target) => n >= target * 0.95 && n <= target * 1.05;
        if (near(5 * 3600))
            return 'rolling';
        if (near(7 * 24 * 3600))
            return 'weekly';
    ...
    const placed = [];
    const place = (raw, fallback) => {
        const win = mapWindow(raw);
        if (win)
            placed.push({ win, lane: windowLane(raw?.limit_window_seconds) ?? fallback });
    };
 ...
 */
let qoderTokenCache = null;
async function exchangeQoderJobToken(base, pat, proxyUrl, timeoutMs) {
...
async function queryQoderUsage(base, token, proxyUrl, timeoutMs) {
...
async function fetchQoderRow(pat, endpoint, proxyUrl, timeoutMs) {
...
const CATALOG = [
    ...
    // the credential service has not resolved the door-knock ref yet (the
    // startup race that made the rows flicker out of the settings list);
    // fetchRow still sends the header when the ref resolves. The host half
...
const FORMATS = {
    'deepseek-balance': (body) => {
        ...
        if (!Number.isFinite(amount))
            throw new Error('missing balance_infos[0].total_balance');
    ...
    'openrouter-credits': (body) => {
        ...
        if (!Number.isFinite(credits) || !Number.isFinite(usage))
            throw new Error('missing data.total_credits / data.total_usage');
    ...
    'siliconflow-balance': (body) => {
        ...
        if (!Number.isFinite(amount))
            throw new Error('missing data.balance');
    ...
    'minimax-remains': (body) => {
        ...
        const pick = (m) => {
            ...
            if (Number.isFinite(pctRemaining))
                return { percent: Math.min(100, Math.max(0, Math.round(100 - pctRemaining))), resetsAt };
            const remaining = num(m.current_interval_remaining_count ?? m.current_interval_remains_count ?? m.current_interval_usage_count);
            if (total > 0 && Number.isFinite(remaining))
                return { percent: Math.min(100, Math.round(((total - remaining) / total) * 100)), resetsAt };
            return null;
        };
        const first = pick(coding) ?? remains.map(pick).find((w) => w !== null);
        if (!first)
            throw new Error('no usable current_interval fields in model_remains');
        const windows = { rolling: first };
    ...
    'xai-credits': (body) => {
        ...
        if (!Number.isFinite(cents))
            throw new Error('missing total.val');
        return { kind: 'balance', amount: Math.abs(cents) / 100 };
    },
    'zhipu-quota': (body) => {
        if (body?.code !== 200)
            throw new Error(`upstream code ${body?.code}: ${body?.msg ?? 'unknown'}`);
        const limits = Array.isArray(body?.data?.limits) ? body.data.limits : [];
        if (limits.length === 0)
            throw new Error('data.limits is empty');
        const text = limits.map((l) => {
            if (l.remaining != null)
                return `${l.remaining}/${l.number ?? '?'}`;
    ...
    'opencode-usage': (body) => {
        ...
        const pick = (key) => {
            ...
            if (!Number.isFinite(percent))
                throw new Error(`missing usage.${key}.percent`);
    ...
    'zai-coding-quota': (body) => {
        ...
        const windows = {};
        if (rolling)
            windows.rolling = mkWin(rolling);
        if (weekly && weekly !== rolling)
            windows.weekly = mkWin(weekly);
        if (time)
            windows.monthly = mkWin(time);
        if (Object.keys(windows).length === 0)
            throw new Error('no TOKENS_LIMIT / TIME_LIMIT / CREDIT_LIMIT entries');
        ...
        const title = [
            plan ? `plan: ${plan}` : null,
            laneTitle('5h', rolling, windows.rolling),
            laneTitle('weekly', weekly, windows.weekly),
    ...
    'kimi-coding-usage': (body) => {
        ...
        if (!weekly && body?.usage) {
            ...
            if (limit > 0 && Number.isFinite(used))
                weekly = { percent: Math.min(100, Math.round((used / limit) * 100)), resetsAt: usage.resetTime };
        }
        if (weekly)
            windows.weekly = weekly;
        if (Object.keys(windows).length === 0)
            throw new Error('no usable windows: limits[].window.duration=300/10080 or usage.limit needed');
        const title = [
    ...
    'antigravity-quota': (body, provider) => {
        if (body?.ok !== true || !body?.value)
            throw new Error('missing ok/value envelope');
        const family = provider?.id === 'antigravity-claude' ? '3p' : provider?.id === 'antigravity-gemini' ? 'gemini' : null;
        if (family === null)
            throw new Error(`provider id must be antigravity-gemini or antigravity-claude, got ${JSON.stringify(String(provider?.id))}`);
        ...
        if (!Array.isArray(body.value.bucketRows)) {
            ...
            if (pool.length === 0)
                throw new Error(`no ${poolTag} rows in value.modelRows`);
        ...
        if (rows.length === 0)
            throw new Error('value.bucketRows is empty');
        const win = (id) => {
        ...
        if (!rolling && !weekly)
            throw new Error(`no ${family}-5h / ${family}-weekly buckets in value.bucketRows`);
        const windows = {};
        if (rolling)
            windows.rolling = rolling;
        if (weekly)
    ...
    // same semantics as a balance amount. rateLimit is the fork-recorded
    // soft-rate (429) window { limited, resetsAt?, lastHitAt? } from the
    // workbuddy plugin: limited=true escalates the row (error dot + sub
    // caption via the optional fields below) and heads the hover title;
    // absent on an unforked upstream response, which renders as before.
    'workbuddy-credits': (body) => {
        if (body?.status !== 'signed-in')
            throw new Error(`auth invalid (status: ${JSON.stringify(String(body?.status ?? 'missing'))})`);
        const amount = Number(body?.credits?.total);
        if (!Number.isFinite(amount))
            throw new Error('missing credits.total');
...
export const Config = z.object({
    ...
    providers: z.array(z.object({
        ...
        region: z.string(),
        localAuth: z.union([z.const('codex'), z.const('loopback')]),
...
function validateProviders(raw) {
    ...
    return raw.providers.map((entry, index) => {
        ...
        if (seen.has(entry.id))
            throw new Error(`${at}.id duplicates "${entry.id}"`);
        seen.add(entry.id);
        const format = entry.format ?? 'deepseek-balance';
        const meta = FORMAT_META[format];
        if (!meta)
            throw new Error(`${at}.format "${format}" is unknown`);
...
function validateProxies(proxies) {
    ...
    for (const [name, url] of Object.entries(proxies || {})) {
        if (typeof url !== 'string')
            throw new Error(`quota-panel: config.proxies.${name} must be a URL string`);
        try {
...
function validateCatalog(catalog, proxies) {
    ...
    for (const [id, override] of Object.entries(catalog || {})) {
        if (!known.has(id))
            throw new Error(`quota-panel: config.catalog.${id} matches no catalog entry (known: ${[...known].join(', ')})`);
        if (!override || typeof override !== 'object')
            throw new Error(`quota-panel: config.catalog.${id} must be an object`);
 ...
 */
function proxiedGetJson(targetUrl, proxyUrl, headers, timeoutMs, method = 'GET', body) {
 ...
 * @returns {Promise<{ok: boolean, status: number, json(): Promise<any>}>}
 */
async function getJson(url, headers, proxyUrl, timeoutMs, method = 'GET', body) {
...
async function resolveRows(ctx, raw) {
    ...
    if (raw.auto) {
        for (const entry of CATALOG) {
            if (raw.hide.includes(entry.id) || explicitIds.has(entry.id))
                continue;
            ...
            if (merged.localAuth) {
                ...
                if (merged.localAuth === 'codex' && !(await chatgptAuthExists()))
                    continue;
                if (merged.localAuth === 'loopback') {
                    // Loopback rows ride local plugin endpoints that need no
                    // credential — the refs are only an optional door-knock
                    // for the Bearer header. Probe them but never gate the
                    // row on the credential service's readiness.
                    const loopbackHit = await firstResolvedRef(ctx, merged.refs);
                    credentialRef = loopbackHit ? loopbackHit.ref : undefined;
                }
                else {
...
async function fetchRow(ctx, provider, proxies, clientProxyUrl) {
    try {
        ...
        const hit = provider.credential === undefined ? null : await ctx.credentials.resolve(provider.credential);
        if (!hit || typeof hit.value !== 'string' || hit.value.length === 0) {
            // Loopback rows tolerate a missing door-knock credential (the
            // endpoint needs none) by fetching without a Bearer header;
            // every other row reports the unconfigured ref as before.
            if (provider.localAuth !== 'loopback') {
        ...
        const headers = hit && typeof hit.value === 'string' && hit.value.length > 0
        ...
        const body = await upstream.json().catch(() => null);
        if (body === null)
            return { id: provider.id, error: `HTTP ${upstream.status}: non-JSON response` };
        const outcome = adaptRow(provider.format, body, provider);
...
function rpcEnvelope(rpcId, result, status = 200) {
 ...
 */
async function rpcResponse(endpoint, request, handle) {
    ...
    try {
        ...
        return rpcEnvelope(message.rpcId, rpcError('internal', String((error && error.message) || error)));
    }
}
/**
...
export function apply(ctx, config = {}) {
    ...
    if (!(refreshMs >= 5000))
        throw new Error('quota-panel: config.refreshMs must be >= 5000');
    ...
    const handleRpc = async (endpoint, payload, _signal) => {
        try {
    ...
    for (const endpoint of RPC_ENDPOINTS) {
        ctx.connection.fetch.register({
            ...
            fetch: (request) => rpcResponse(endpoint, request, handleRpc)
        });
    }
}
