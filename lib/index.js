...
export function apply(ctx, config = {}) {
    ...
    ctx.effect(() => {
        const disposers = RPC_ENDPOINTS.map((endpoint) => ctx.connection.fetch.register({
            path: rpcRoutePath(endpoint),
            methods: ['POST'],
            requestBody: 'buffered',
            fetch: (request) => rpcResponse(endpoint, request, (ep, payload) => dispatchRpc(service, ep, payload))
        }));
        return () => {
