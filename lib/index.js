...
const FORMATS = {
    ...
    'antigravity-quota': (body, provider) => {
        ...
        if (body.value.authenticated !== true)
            throw new Error(`not authenticated (login: ${String(body.value.login?.status ?? 'unknown')})`);
        const quota = body.value.quota;
        const rows = Array.isArray(quota?.bucketRows) ? quota.bucketRows : [];
        if (rows.length === 0)
            throw new Error('quota.bucketRows is empty');
        ...
        if (!rolling && !weekly)
            throw new Error(`no ${family}-5h / ${family}-weekly buckets in quota.bucketRows`);
        const windows = {};
        if (rolling)
            windows.rolling = rolling;
        if (weekly)
