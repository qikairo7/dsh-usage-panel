...
(window as any).__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			'#dsh-quota-capsule .dsh-capsule-name{font-size:11.5px;font-weight:600;letter-spacing:-0.01em;color:var(--dsw-alias-label-secondary,#61666b);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:64px}',
			...
			'@media (prefers-reduced-motion: reduce){#dsh-quota-capsule,#dsh-quota-card,#dsh-quota-card .dsh-quota-settings,#dsh-quota-card .dsh-setting-section,#dsh-quota-card .dsh-progress-fill,#dsh-quota-card .dsh-quota-icon,#dsh-quota-card .dsh-setting-reset,#dsh-quota-capsule .dsh-capsule-dot,#dsh-quota-capsule .dsh-capsule-name,#dsh-quota-capsule .dsh-capsule-more,#dsh-quota-card .dsh-status-dot,#dsh-quota-card .dsh-setting-toggle-track,#dsh-quota-card .dsh-setting-toggle-thumb,#dsh-quota-card .dsh-setting-seg-btn{animation:none;transition:none;transform:none}}'
		].join("\n");
			...
			React.useEffect(function () {
				...
				if (!expanded) {
					...
					if (specs === null && loadError !== null) {
						...
						var visibleRows = sortedRows.slice(0, MAX_CAPSULE_VISIBLE);
						var overflowRows = sortedRows.slice(MAX_CAPSULE_VISIBLE);

						for (var j = 0; j < visibleRows.length; j++) {
							var rspec = visibleRows[j];
							...
							pairs.push(React.createElement("span", { key: rspec.id + "-group", className: "dsh-capsule-group" },
								...
								React.createElement("span", { key: "name", className: "dsh-capsule-name" }, rspec.short || rspec.label),
						...
						if (overflowRows.length > 0) {
							...
							var moreState = hasErr ? "error" : hasWarn ? "warn" : "ok";
