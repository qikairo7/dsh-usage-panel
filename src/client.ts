...
(window as any).__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			'#dsh-quota-capsule{display:flex;flex-wrap:wrap;align-items:center;row-gap:6px;column-gap:12px;max-width:264px;max-width:min(264px,calc(100vw - 36px));box-sizing:border-box;padding:10px 14px;border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,0.08));border-radius:18px;background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,0.85));backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:var(--dsw-shadow-lv2,0 2px 8px rgba(0,0,0,0.04),0 1px 2px rgba(0,0,0,0.02));cursor:pointer;font:inherit;outline:none;user-select:none;touch-action:none;transform:translateY(0);transition:transform 180ms cubic-bezier(0.16,1,0.3,1),box-shadow 180ms ease,background-color 150ms ease,border-color 150ms ease;animation:dsh-quota-capsule-enter 200ms cubic-bezier(0.16,1,0.3,1)}',
			...
			'#dsh-quota-capsule .dsh-capsule-name{font-size:10.5px;font-weight:600;letter-spacing:-0.01em;color:var(--dsw-alias-label-secondary,#61666b);white-space:nowrap}',
			'#dsh-quota-capsule .dsh-capsule-ring{display:inline-block;flex:none;width:10px;height:10px;border-radius:50%;--rp:var(--dsh-ring-pct,0);--rc:var(--dsw-static-neutral-bluish-400,#adb2b8);background:conic-gradient(var(--rc) calc(var(--rp) * 1%), var(--dsw-alias-bg-raised,rgba(0,0,0,0.08)) 0);-webkit-mask:radial-gradient(circle,transparent 0 3px,#000 3px);mask:radial-gradient(circle,transparent 0 3px,#000 3px);transition:background-color 160ms ease}',
			...
			'#dsh-quota-capsule .dsh-capsule-item{font-size:11.5px;font-weight:550;font-variant-numeric:tabular-nums;letter-spacing:-0.01em;white-space:nowrap;color:var(--dsw-alias-label-primary,#1b1b1c);transition:color 150ms ease;flex-shrink:0}',
						...
						for (var j = 0; j < sortedRows.length; j++) {
							...
							pairs.push(React.createElement("span", { key: rspec.id + "-group", className: "dsh-capsule-group" },
								React.createElement("span", {
									key: "ring",
									className: "dsh-capsule-ring state-" + (rview.status || "ok"),
									style: { "--dsh-ring-pct": pct }
								}),
								React.createElement("span", { key: "name", className: "dsh-capsule-name" }, rspec.short || rspec.label),
								React.createElement("span", {
									key: "value",
									className: "dsh-capsule-item state-" + (rview.status || "ok") + (rview.kind === "usage" ? " dsh-usage" : "")
								}, rview.summary)
							));
						}
						var ariaParts = [];
