...
(window as any).__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			'@media (prefers-reduced-motion: reduce){#dsh-quota-capsule,#dsh-quota-card,#dsh-quota-card .dsh-quota-settings,#dsh-quota-card .dsh-setting-section,#dsh-quota-card .dsh-progress-fill,#dsh-quota-card .dsh-quota-icon,#dsh-quota-card .dsh-setting-reset,#dsh-quota-capsule .dsh-capsule-dot,#dsh-quota-card .dsh-status-dot,#dsh-quota-card .dsh-setting-toggle-track,#dsh-quota-card .dsh-setting-toggle-thumb,#dsh-quota-card .dsh-setting-seg-btn{animation:none;transition:none;transform:none}}'
		].join("\n");
				...
				if (!expanded) {
					...
					if (specs === null && loadError !== null) {
						...
						for (var j = 0; j < rows.length; j++) {
							...
							pairs.push(React.createElement("span", { key: rspec.id + "-group", className: "dsh-capsule-group" },
								React.createElement("span", { key: "dot", className: "dsh-capsule-dot state-" + view.status }),
