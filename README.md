...
- **v0.8.1-rc.6** — layout fixes from issue #1, reworked: the panel is now
  **draggable** — grab the collapsed capsule or the expanded card header and
  ...
  still works, viewport-clamped so it stays grabbable, position persisted to
  localStorage with the other settings, restored positions re-clamped on
  resize, "restore defaults" clears it). The default anchor stays
  right/bottom 18px until the first drag. Also injects
  `[class*="overlayLayer"]{z-index:1150 !important}` so the shell overlay
  ...
  version is an orphan pre-release.)
